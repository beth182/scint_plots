...
processed_wx_path = '//rdg-home.ad.rdg.ac.uk/research-nfs/rds/micromet/Tier_processing/rv006011/new_data_scint/'
...
for index, row in days_df.iterrows():
    ...
    wx_files = find_files(site='BCT',
                          instrument='Davis', level='L2', time_res='1min',
                          main_dir=processed_wx_path, doy=doy)

    assert len(wx_files['file_paths']) == 1
    raw_file = wx_files['file_paths'][0]

    shutil.copy(raw_file, 'D:/zenodo/P2 CASES/AWS_processed/')
    print(raw_file)
