...
temp_path_lon = pd.DataFrame.from_dict(
    ({'lon': [df_las_lon.index.max() + diff, df_las_lon.index.min() - diff], 'y_crosssec_to1_path': [np.nan, np.nan]}))
...
ax2 = ax.twiny()
# east to west
las_lat = ax2.plot(df_las_lat.index - min_lat, df_las_lat.x_crosssec_to1_path, label='LAS cross-wind', color='green',
                   linestyle='-')
...
# north to south
las_lon = ax.plot(df_las_lon.index - min_lon, df_las_lon.y_crosssec_to1_path, label='LAS along-wind', color='magenta',
                  linestyle='-')
...
leg = ax.legend(lns, labs, loc='center left', bbox_to_anchor=(1, 0.5), title="Cross Sections", labelcolor='linecolor')
