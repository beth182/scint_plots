...
for DOY in DOY_list:
    ...
    ukv_data_dict_wind = retrieve_model_fluxes.retrieve_UKV(scint_path, DOY, DOY, variable='wind',
                                                            av_disheight=np.nanmean(df['z_f']) - np.nanmean(df['z_0']))
    UKV_df = retrieve_model_fluxes.UKV_df(ukv_data_dict_wind, wind=True)
